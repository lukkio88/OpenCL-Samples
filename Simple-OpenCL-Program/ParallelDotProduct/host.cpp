...
bool runCL(double * a, double * b, double * c, const int & n) {
	...
	cl_mem c_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double), nullptr, nullptr);
	...
	kernel[0] = clCreateKernel(program[0], "dotProduct", &err);
	...
	if (check_error(clSetKernelArg(kernel[0], 1, sizeof(cl_mem), &b_mem), "Error in clSetKernelArg for b_mem"))
		return true;
	if (check_error(clSetKernelArg(kernel[0], 2, n*sizeof(double), NULL), "Error in clSetKernelArg for localMem"))
		return true;
	if (check_error(clSetKernelArg(kernel[0], 3, sizeof(cl_mem), &c_mem), "Error in clSetKernelArg for c_mem"))
		return true;
	...
	if (check_error(clEnqueueReadBuffer(cmd_queue, c_mem, CL_TRUE, 0, sizeof(double), c, 0, nullptr, nullptr), "Error in clEnqueueReadBuffer"))
		return true;
	clFinish(cmd_queue);
...
int main(int argc, char **argv) {
	...
	/*
	 * this will be used for both parallel product and partial sum, the first entry will store
	 * the final result
	 */
	for (auto i = 0; i < n; ++i) {
